vim.keymap.set("n", "<Esc>", "<cmd>nohlsearch<CR>", { desc = "Set highlight on search, <Esc> clears in normal mode" })
...
vim.keymap.set("n", "<A-k>", "<Cmd>try | cprevious | catch | clast | catch | endtry<CR>zz")
...
--NOTE: requires a key redirect mapping in terminal emulator
vim.keymap.set("n", "<F13>", "<C-I>", { noremap = true, silent = true, desc = "F13 behave like ctrl+i" })
...
-- stylua: ignore
vim.keymap.set( "n", "k", [[(v:count > 1 ? "m'" . v:count : "g") . 'k']], { expr = true, desc = "Add rel jumps to the jumplist" })
-- stylua: ignore
